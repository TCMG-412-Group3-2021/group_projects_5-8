...
def test_prime_test():
    prime_test = requests.get(f'http://{hostName}:80/is-prime/3').json()
    result= {'input': 3, 'output': True}
    assert prime_test == result

def test_factorial_test():
    factorial_test = requests.get(f'http://{hostName}/factorial/9').json()
    result= {"input":9,"output":362880}
    assert factorial_test == result

def test_md5_test():
    md5_test = requests.get(f'http://{hostName}/md5/hello').json()
